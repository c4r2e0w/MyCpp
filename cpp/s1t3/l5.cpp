...
//Тело функции сортировки увеличивает размеры содержащей её функции, и усложняет чтение кода. 
//Поэтому рекомендация остаётся прежней: старайтесь делать лямбда функции как можно короче. 
//Если обычная функция сделает код понятнее, отдавайте предпочтение ей. 
//Например, предыдущая программа только выиграет от замены лямбда-функции на обычную:
